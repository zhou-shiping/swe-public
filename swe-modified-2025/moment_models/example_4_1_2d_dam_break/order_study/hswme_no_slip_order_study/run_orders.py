#!/usr/bin/env python3
"""Run the no-slip classical HSWME retained-order study."""
...
from high_order_hswme import solve_hswme
...
def output_path(directory: Path, order: int) -> Path:
    return directory / f"HSWME_N{order}_data_kappa1.0e+10.npy"
...
def main() -> None:
    ...
    for order in orders:
        ...
        print(f"Running no-slip HSWME N={order}", flush=True)
    ...
    available_runs = []
    for path in sorted(args.output_dir.glob("HSWME_N*_data_kappa1.0e+10.npy")):
    ...
    metadata = {
        "study": "classical HSWME retained-order sweep, no-slip bottom",
