...
def Initial(Nx, Ny, dx, dy, H_center, H_rest, var_num, U1, H1, init_velocity):
    """Initialize h, hu, halpha1,..., halphaN for the dam break example
    ...
    """
    # 2D SWE
    # U = [h, hu, hv, halpha1, hbeta1, halpha2, hbeta2, ..., halphaN, hbetaN]^{T}
    
    U = np.zeros((Nx, Ny, var_num)) # U=[h,hu,halpha1,...,halphaN]^{T}
    #SourceU = np.zeros((Nx, Ny, var_num))
    #RHS = np.zeros((Nx, Ny, var_num))
    ...
    for i in range(Nx):
        ...
        for j in range(Ny):
            ...
            #if (xi-x_center)**2 <= radius**2:
                U[i,j,0] = H_center
                if init_velocity == 'nonzero':
                    U[i,j,1] = 0.125*U[i,j,0]**2 * H1/U1 #hu
                    U[i,j,2] = 0.125*U[i,j,0]**2 * H1/U1 #hv
                if var_num == 5 and init_velocity == 'nonzero':
                    U[i,j,3] = -0.125*U[i,j,0]**2 * H1/U1 #halpha1
                    U[i,j,4] = -0.125*U[i,j,0]**2 * H1/U1 #hbeta1
                elif var_num == 7 and init_velocity == 'nonzero':
                    U[i,j,3] = -0.125*U[i,j,0]**2 * H1/U1 #halpha1
                    U[i,j,4] = -0.125*U[i,j,0]**2 * H1/U1 #hbeta1
                    U[i,j,5] = 0.0 #halpha2
                    U[i,j,6] = 0.0 #hbeta2
...
def SWME_Solver(U1, H1, H_center, H_rest, L, G, Nx, Ny, CFL, time_end,
    ...
    dy = L/Ny
    # Initialization
    U = Initial(Nx, Ny, dx, dy, H_center, H_rest, var_num, U1, H1, init_velocity=init_case)
